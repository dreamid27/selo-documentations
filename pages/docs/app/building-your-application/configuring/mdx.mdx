...
Next.js mendukung kompiler MDX baru yang ditulis dengan Rust. Kompiler ini masih dalam tahap eksperimental dan tidak disarankan untuk digunakan di produksi. Untuk menggunakan kompiler baru ini, Anda perlu mengonfigurasi `next.config.js` saat Anda meneruskannya ke `withMDX`:
