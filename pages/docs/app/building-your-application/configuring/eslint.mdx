...
Secara default, Next.js akan menjalankan ESLint untuk semua file dalam direktori `pages/`, `app/`, `components/`, `lib/`, dan `src/`. Namun, Anda dapat menentukan direktori mana yang akan digunakan opsi `dirs` dalam konfigurasi `eslint` di `next.config.js` untuk build produksi:

```js filename="next.config.js"
module.exports = {
  eslint: {
    dirs: ['pages', 'utils'], // Hanya menjalankan ESLint pada direktori 'pages' dan 'utils' selama build produksi (next build)
  },
};
```
