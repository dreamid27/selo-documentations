...
Komponen kesalahan dapat menggunakan fungsi `reset()` untuk mendorong pengguna untuk mencoba pulih dari kesalahan. Saat dieksekusi, fungsi tersebut akan mencoba untuk merender ulang konten Batasan Kesalahan. Jika berhasil, komponen kesalahan pengganti digantikan oleh hasil dari penggambaran ulang.
...
### Menangani Kesalahan dalam Tata Letak
...
### Menangani Kesalahan dalam Tata Letak Akar
...
Jika terjadi kesalahan dalam Server Component, Next.js akan meneruskan objek `Error` (yang telah dihilangkan informasi kesalahan sensitifnya saat produksi) ke file `error.js` terdekat sebagai prop `error`.

#### Menyelamatkan Informasi Kesalahan Sensitif

Selama produksi, objek `Error` yang diteruskan ke klien hanya mencakup properti `message` dan `digest` yang bersifat generik.
